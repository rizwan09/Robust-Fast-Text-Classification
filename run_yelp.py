...
# run_command = 'python3 '+pyfile+ ' --task yelp --num_class 5 --gpu '+str(gpu) + ' --batch_size 32 --dropout '+str(dp)+ ' --model_file_name stop_full_ori.pth.tar '+ ' --early_stop 2' 
# print (run_command)
...
sparsity_list = [0.00005]#[0.1, 0.25, 0.025, 0.05, 0.075, 0.00075, 0.0005, 0.00025] # [0.5 , 0.05, 0.02, 0.015, 0.01, 0.001, 0.00075,  0.0005, 0.00025, 0.0001]
coherent_list = [9.0]
debug=0
		...
		for coherent in coherent_list:
			...
			if WAG==1:
				if full_classifier ==1: 
					...
					model_file_name = 'full_WAG_classifier_nlp'+'_dp_'+str(dp)+'.pth.tar'
				else:
			...
			options+=' --model_file_name '+model_file_name +' --load_model ' + str(load_model)+' --early_stop 2 --num_class 5'+\
