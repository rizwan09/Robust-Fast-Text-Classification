...
# ## python3 bcn_rt_main.py --task RT --gpu 1 --batch_size 32 --dropout 0.2 --num_units 4 80.80
# nlayers = 2
...
# nhid = 200
# for n in [4]:
# 	run_command = 'python3 '+pyfile+ ' --task RT --gpu '+str(gpu) + ' --batch_size 16 --dropout '+str(dp)+ ' --num_units '+str(n) + ' --model_file_name full_ori.pth.tar'  #+' --nlayers '+str(nlayers) #+' --nhid '+ str(nhid)
# 	print (run_command)
...
sparsity_list = [0]#[0.00075,  0.0005, 0.00025, 0.0001, 0.000075,  0.00005, 0.000025, 0.00001]#[0.1, 0.25, 0.5, 0.75, 0.025, 0.05, 0.075, 0.00025, 0.00075, 0.0005] # [0.5 , 0.05, 0.02, 0.015, 0.01, 0.001, 0.00075,  0.0005, 0.00025, 0.0001]
coherent_list = [0]#[2.0, 5.0] #[2.0,5.0]
debug=0
...
for dp in dropout:
	for sparsity in sparsity_list:
		for coherent in coherent_list:
			...
			options+=' --model_file_name '+model_file_name +' --load_model ' + str(load_model)+' --num_units 4 --batch_size 16 '+\
			...
			if save_selection==1: 
				...
				options+= ' --save_selection '+ str(save_selection) +' --batch_size 16' + ' --selector_file_name '+selector_file_name
