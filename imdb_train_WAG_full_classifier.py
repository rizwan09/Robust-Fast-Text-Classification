...
class Train:
        ...
        for epoch in range(start_epoch, start_epoch + n_epochs):
            if not self.stop:
                ...
                if self.best_dev_acc < dev_acc and dev_acc!=-1:
                    ...
                    # }, self.config.output_base_path+ self.config.task+'/'+ 'model_best.pth.tar')
    ...
    def train(self, train_corpus, epoch):
        ...
        shuffle = True #self.config.WAG==0 For shuffle
        print (shuffle)
        train_batches = helper.batchify(train_corpus.data, self.config.batch_size, shuffle = shuffle)
