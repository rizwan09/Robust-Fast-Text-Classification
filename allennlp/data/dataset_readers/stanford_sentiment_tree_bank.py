...
class StanfordSentimentTreeBankDatasetReader(DatasetReader):
    ...
    def _read(self, file_path):
        with open(cached_path(file_path), "r") as data_file:
            logger.info("This reader code is only for testing please edit for training:")
            fp = '../bcn_output/sst/predicted_text_words.txt'
            logger.info("Reading instances from lines in file at: %s", fp)
            ...
            for line in data_file.readlines():
                ...
                if self._use_subtrees:
                    for subtree in parsed_line.subtrees():
                        if len(subtree.leaves())<=1: continue
                        instance = self.text_to_instance(subtree.leaves(), subtree.label())
                        if instance is not None:
                            yield instance
                        if len(subtree.leaves())<=1: continue
                        for idx in range(len(subtree.leaves())-1):
                            instance = self.text_to_instance(subtree.leaves()[idx:idx+1], subtree.label())
                            if instance is not None:
                                yield instance
                else:
                    width = 5
                    words = []#[ w for w in parsed_line.leaves() if w not in stop_words] #[word for word in parsed_line.leaves() if word.lower() in selected_words ]
                    for i in range(len(parsed_line.leaves())):
                        word =  parsed_line.leaves()[i]
                        # print("i", i, word, word.lower() in selected_words)
                        if word.lower() in selected_words:
                            for j in range(i-(width),  i+(2*width+1)):
                                if j<0 or j>=len(parsed_line.leaves()): continue
                                # print('j:', j, parsed_line.leaves()[j])
                                # print(parsed_line.leaves()[j] not in words, words)
                                if parsed_line.leaves()[j] not in words:
                                    words.append(parsed_line.leaves()[j])
                            i+=2

                    if len(words)<1:words = parsed_line.leaves()
                    # print(parsed_line.leaves(), " and: ", words)
                    instance = self.text_to_instance(words, parsed_line.label())
                    if instance is not None:
                        yield instance
                # exit()
        ...
        # if self._use_subtrees:
        #     WAG_FILE = '../bcn_output/sst/WAG_full_train.txt'
        #     with open(cached_path(WAG_FILE), "r") as data_file:
        #         logger.info("Reading instances from lines in file at: %s", WAG_FILE)
        #         c = 0
        #         for line in data_file.readlines():
        #             line = line.strip("\n")
        #             if not line:
        ...
        #             if self._use_subtrees:
        #                 for subtree in parsed_line.subtrees():
        #                     if len(str(subtree.label())) == 0: continue
        #                     instance = self.text_to_instance(subtree.leaves(), labels[c]) #subtree.label())
        #                     if instance is not None:
        #                         yield instance
        #                         c+=1
        #                         if c == len(labels):
