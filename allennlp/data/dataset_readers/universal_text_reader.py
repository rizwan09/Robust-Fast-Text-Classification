...
@DatasetReader.register("universal_text_reader")
class UniversalDatasetReader(DatasetReader):
    ...
    def _read(self, file_path):
        with open(cached_path(file_path), "r", encoding='utf-8', errors='ignore') as data_file:
            ...
            for line in data_file.readlines():
                ...
                parsed_line = line[1:]
                parsed_label = line[0]
                # parsed_label = str(int(line[0])-2)
                # parsed_label = str(int(line[0])-3)
                # parsed_label = str(int(line[0])+1)
                # parsed_label = str(int(line[0])-1)
                ...
                # if parsed_label not in ['0', '1']: 
                #     i+=1
                #     print(" i: ", i, line, parsed_line, parsed_label)
                #     exit()
                instance = self.text_to_instance(parsed_line, parsed_label)
                if instance is not None:
                    yield instance

        # if 'train' in file_path:
        #     WAG_FILE = '../bcn_output/RT/dev.txt'
        ...
        #             parsed_line = line[1:]
        #             parsed_label = line[0]
        #             print(str(parsed_label)+"\t"+' '.join(parsed_line))
        #             exit()
        #             instance = self.text_to_instance(parsed_line, parsed_label)
        #             if instance is not None:
        #                 yield instance
