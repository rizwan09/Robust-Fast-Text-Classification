...
from allennlp.utils.nn_layer import WE_Selector
...
from allennlp.models.selector import Selector
=======
>>>>>>> 9b2f0b45abdad09c78036fdaebe7dd2c9973128a
...
>>>>>>> 9b2f0b45abdad09c78036fdaebe7dd2c9973128a
    def __init__(self,
                 ...
                 optimizer_selector: torch.optim.Optimizer,
...
<<<<<<< HEAD
                 ...
                 learning_rate_scheduler_selector: Optional[PytorchLRScheduler] = None,
=======
...
<<<<<<< HEAD
        ...
        self._optimizer_selector = optimizer_selector
        self._cuda_device = cuda_device
        
        self._train_data = train_dataset#[10000:10320]
        self._validation_data = validation_dataset#[1200:1840]
=======
        self.model = model
...
>>>>>>> 9b2f0b45abdad09c78036fdaebe7dd2c9973128a
        ...
        self._learning_rate_scheduler_selector = learning_rate_scheduler
...
<<<<<<< HEAD
    def _rescale_gradients_selector(self) -> Optional[float]:
        """
        Performs gradient rescaling. Is a no-op if gradient rescaling is not enabled.
        """
        if self._grad_norm:
            parameters_to_clip = [p for p in self._selector.parameters()
                                  if p.grad is not None]
            return sparse_clip_norm(parameters_to_clip, self._grad_norm)
        return None
...
<<<<<<< HEAD
            logpz = zsum = zdiff =   0.0
            selected = total = 1.0

            # print(' sent size: ', batch['tokens']['tokens'].size(), ' embeded size: ', dropped_embedded_text.size())

            # selected_x, logpz, zsum, zdiff = self._selector(**batch, is_train=for_training)
            # zsum=zsum.mean()
            # zdiff =zdiff.mean()
            # logpz =logpz.mean()
            

            # batch['tokens']['tokens'] = selected_x

            # selected = selected_x.size()[0] * selected_x.size()[1]
            ...
            selected = output_dict["skimmed_words"]
            total = output_dict["total_words"]
=======
...
>>>>>>> 9b2f0b45abdad09c78036fdaebe7dd2c9973128a
        ...
        return loss, logpz, zsum, zdiff, selected, total
...
<<<<<<< HEAD
            ...
            # self._optimizer_selector.zero_grad()


            loss, logpz, zsum, zdiff, selected, total = self._batch_loss(batch, for_training=True)
            nselected+=selected
            ntotal+=total
            # import pdb
            # if epoch==1: pdb.set_trace()
            # loss.backward(retain_graph=True)
            loss.backward()

            # coherent_factor = sparsity * coherent
            # z = zsum * sparsity
            # d = zdiff * coherent_factor
            # s =(z + d)

            # # print('s: ', s, ' loss: ', loss)

            # cost_vec = loss + s
            # cost_logpz = (cost_vec * logpz)
            # cost_logpz.backward()
            ...
            # batch_grad_norm_selector = self._rescale_gradients_selector()


            self._update_learning_rate(None, batch_num_total=batch_num_total)
            # self._update_learning_rate_scheduler(None, batch_num_total=batch_num_total)
...
<<<<<<< HEAD
                ...
                param_updates_selector = {name: param.detach().data.cpu().clone()
                                 for name, param in self._selector.named_parameters()}
                
                self._optimizer.step()
                # self._optimizer_selector.step()
...
>>>>>>> 9b2f0b45abdad09c78036fdaebe7dd2c9973128a
                    ...
                    self._tensorboard.add_train_scalar("gradient_update/" + name,
                                                       ...
                                                       batch_num_total)
                    # param_updates_selector[name].sub_(param.detach().data.cpu())
                    # update_norm_selector = torch.norm(param_updates[name].view(-1, ))
                    # param_norm_selector = torch.norm(param.view(-1, ))
                    # self._tensorboard.add_train_scalar("gradient_update/" + name,
                                                       # update_norm_selector / (param_norm_selector + 1e-7),
                                                       # batch_num_total)
            else:
<<<<<<< HEAD
                self._optimizer.step()
...
<<<<<<< HEAD
                ...
                # self._parameter_and_gradient_statistics_to_tensorboard(batch_num_total, batch_grad_norm_selector)
=======
...
<<<<<<< HEAD
        ...
        logger.info(" skimmed: "+str(nselected)+ "/"+ str(ntotal)+" percentge: " + str(1.0*nselected/ntotal))
...
>>>>>>> 9b2f0b45abdad09c78036fdaebe7dd2c9973128a
    ...
    def _update_learning_rate_scheduler(self, epoch: int, val_metric: float = None,
                              batch_num_total: int = None) -> None:
        if not self._learning_rate_scheduler:
            return

        if batch_num_total is not None:
            if hasattr(self._learning_rate_scheduler_selector, 'step_batch'):
                self._learning_rate_scheduler_selector.step_batch(batch_num_total)
            return
        ...
        # reduce_on_plateau = isinstance(self._learning_rate_scheduler_selector, torch.optim.lr_scheduler.ReduceLROnPlateau)
        ...
        #     self._learning_rate_scheduler_selector.step(val_metric, epoch)
        # else:
        #     self._learning_rate_scheduler_selector.step(epoch)
...
<<<<<<< HEAD
            # ntokens += batch['passage']['tokens'].size(0)*batch['passage']['tokens'].size(1)

            loss, logpz, zsum, zdiff, selected, total = self._batch_loss(batch, for_training=False)
            val_loss += loss.data.cpu().numpy()
            ntotal+=total
            nselected+=selected
            # all_rep.append(rep.tolist())
=======
...
>>>>>>> 9b2f0b45abdad09c78036fdaebe7dd2c9973128a
        ...
        # logger.info("total number of words: " + str(ntokens))
...
<<<<<<< HEAD
        if cuda_device >= 0:
            ...
            selector = selector.cuda(cuda_device)
            # model.set_cuda_device(cuda_device)
=======
        if isinstance(cuda_device, list):
...
>>>>>>> 9b2f0b45abdad09c78036fdaebe7dd2c9973128a
        ...
        parameters_selector = [[n, p] for n, p in selector.named_parameters() if p.requires_grad]
        optimizer_selector = Optimizer.from_params(parameters_selector, params.pop("optimizer_selector"))


        if lr_scheduler_params:
            scheduler = LearningRateScheduler.from_params(optimizer, lr_scheduler_params)
            scheduler_selector = LearningRateScheduler.from_params(optimizer_selector, lr_scheduler_params)
        else:
            scheduler = None
...
<<<<<<< HEAD
        return Trainer(model, selector, optimizer, optimizer_selector, iterator,
                       ...
                       learning_rate_scheduler_selector=scheduler_selector,
