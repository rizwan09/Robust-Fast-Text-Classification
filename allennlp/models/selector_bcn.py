...
@Model.register("selector_bcn")
class SelectorBiattentiveClassificationNetwork(Model):
    """
    ...
    def __init__(self,
        ...
        super(SelectorBiattentiveClassificationNetwork, self).__init__(vocab, regularizer)
    ...
    def forward(self,  # type: ignore
        ...
        'rep': pooled_representations_dropped}
        if label is not None:
            loss = self.loss(logits, label.squeeze(-1))
            for metric in self.metrics.values():
                metric(logits, label.squeeze(-1))
            output_dict["loss"] = loss
            output_dict["skimmed_words"] = 0
            output_dict['total_words'] = 1
            output_dict['time_needed'] = 0
