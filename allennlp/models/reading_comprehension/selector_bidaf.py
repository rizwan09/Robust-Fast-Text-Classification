...
from torch.nn.functional import nll_loss, sigmoid
...
@Model.register("selector_bidaf")
class SelectorBidirectionalAttentionFlow(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        super(SelectorBidirectionalAttentionFlow, self).__init__(vocab, regularizer)
    ...
    def forward(self,  # type: ignore
        ...
        result_x = passage['tokens'].mul(selection_x) #word ids that are selected; contains zeros where it's not selected (ony selected can be found by selected_x[selected_x!=0])
        char_result_x = passage['token_characters'] * selection_x.unsqueeze(2).repeat(1,1,passage['token_characters'].size()[2])
        # result_y = sentence2.mul(selection_y) 
        # print('result_x: ', result_x)

        selected_x, char_selected_x = helper.get_selected_tensor(result_x, char_result_x, pbx, passage['tokens'], passage['token_characters'], self.cuda_device) #sentence1_len is a numpy array
        print(' passage size: ', passage['tokens'], ' char_passage size: ', passage['token_characters'], ' selected_x: ', selected_x, ' char_selected_x: ', char_selected_x)
        ...
        if for_training:
            mask1 = (passage['tokens']!=self._vocab.get_token_index(DEFAULT_PADDING_TOKEN)).long()
    ...
    @classmethod
    def from_params(cls, vocab: Vocabulary, params: Params) -> 'SelectorBidirectionalAttentionFlow':
